...
export default function SidebarMenu() {
  ...
  const mapItems = Menu.map((item, idx) => (
    <div key={idx} className="w-full">
        ...
        {item.links && (
          <div className="ml-auto">
            <div className={currentIdx === idx ? 'hidden' : 'block'}>
              <svg className="w-6 h-6 ml-auto text-green-100" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                <path
                  fill-rule="evenodd"
            ...
            <div className={currentIdx !== idx ? 'hidden' : 'block ml-auto'}>
              <svg className="w-6 h-6 text-green-100" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                <path
                  fill-rule="evenodd"
      ...
      <div className={currentIdx === idx ? 'block p-8' : 'hidden'}>
        <button className={links}>{item.links && item.links.first}</button>
        <button className={links}>{item.links && item.links.second}</button>
      </div>
    </div>
  ));
